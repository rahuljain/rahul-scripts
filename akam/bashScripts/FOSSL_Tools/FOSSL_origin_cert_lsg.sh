...
if [[ ! -e ${ORIGIN_HOSTS_FILE} ]]; then                                                                                                                                                                                                 
   ...
   echo "Usage: $0 <origin_list_file> <siteshield ip>"
   echo "Example: $0 origin_list.txt 184.84.242.11"
   echo
   exit 1
fi
...
if [[ ! ${SITESHIELD} ]]; then
   echo
   echo "You did not specify an edge server to use"
   echo "Usage: $0 <origin_list_file> <siteshield ip>"
   echo "Example: $0 origin_list.txt 184.84.242.11"
   echo
   exit 1
...
#echo | nsh 216.246.75.19 s_client -connect ${HOST}:443 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ${HOST}_CERTIFICATES/${HOST}_CERT.pem
#echo | nsh 216.246.75.19 s_client -showcerts -connect ${HOST}:443 2>/dev/null |sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ${HOST}_CERTIFICATES/${HOST}_CA.pem
#### Normandy Shell version
echo | nsh 216.246.75.19 s_client --ssl3 ${HOST} 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ${HOST}_CERTIFICATES/${HOST}_CERT.pem
echo | nsh 216.246.75.19 s_client --showcerts ${HOST} 2>/dev/null |sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ${HOST}_CERTIFICATES/${HOST}_CA.pem
